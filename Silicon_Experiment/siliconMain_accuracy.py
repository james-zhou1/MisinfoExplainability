...
match_evaluations = [entry['evaluation'] for entry in dataset if entry['match']]
non_match_evaluations = [entry['evaluation'] for entry in dataset if not entry['match']]

# Calculate average evaluation for matches and non-matches
avg_match_evaluation = np.mean(match_evaluations)
avg_non_match_evaluation = np.mean(non_match_evaluations)
